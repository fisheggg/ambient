...
class Position(object):
    def __init__(self, x1: float, x2: float, x3: float, c_type: str):
        assert c_type.lower() in ["cartesian", "polar"]

        self.x, self.y, self.z = 0., 0., 0.
        self.phi, self.nu, self.r = 0., 0., 0.
        if c_type.lower() == "cartesian":
...
class MovingSource(PositionalSource):
    def __init__(self, signal, positions, rate=44800):
        ...
        self.pts_idx = np.floor(np.linspace(0, (self.npts - 1), self.nframes)).astype(
            int
        )
        self.cur_idx = -1
