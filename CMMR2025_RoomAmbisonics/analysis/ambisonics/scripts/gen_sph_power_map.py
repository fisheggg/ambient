import sys, argparse
import numpy as np
from pyutils.ambisonics.distance import (
    SphericalAmbisonicsVisualizer,
    SphericalSourceVisualizer,
)
...
def run(input_fn, output_fn, position_fn="", angular_res=""):
    ...
    if position_fn:
        srcVis = SphericalSourceVisualizer(
            position_fn, duration, ambiVis.visualization_rate(), angular_res=angular_res
        )
...
def parse_arguments():
    ...
    parser.add_argument(
        "output_fn", help="Output video filename to store spherical power map."
    )
    parser.add_argument(
        "--position_fn",
        default="",
        help="Ground-truth position file. Source locations will be superimposed.",
    )
    parser.add_argument(
        "--angular_res", default=10.0, type=float, help="Angular resolution."
    )
